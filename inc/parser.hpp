...
class Parser{
    private:
        ...
        size_t _inst_no {0};
        size_t _line_no {0};
        void _parse_literal(const Token& token);
        void _parse_word(const Token& token);
        void _parse_inst(const Token& token);
        void _parse_label(const Token& token);
        void _parse_type(const Token& token);
    public:
        Parser() {}
